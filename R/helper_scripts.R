...
check_input_crps_para <- function(crps.para, f.g) {
  ...
  if (!("mu" %in% names(crps.para)) && !("points.cdf" %in% names(crps.para)) && !("sd" %in% names(crps.para)) && !("fun" %in% names(crps.para)) && !("crps.fun.y.matrix" %in% names(crps.para)) && !("sample.fun" %in%names(crps.para)) && !("inf.fun" %in% names(crps.para)) && !("method" %in% names(crps.para))) {
    stop(sprintf("The CRPS parameter ('mu','sd') or ('points.cdf') should not be empty for %s or provide the 4 functions and the method.", f.g))
  }
  ...
  } else if ("points.cdf" %in% names(crps.para)){
...
#'
#' @export
create_crps_fun <- function(mu = 0, sd = 1, w = 1, points.cdf = NA, ...) {
  input <- list(...)
  if ("fun" %in% names(input)) {
    method <- input$method
    crps.fun <- input$fun
    sample.fun <- input$sample.fun
    if (!("crps.fun.y.matrix" %in% names(input))) {
      crps.fun.y.matrix <- input$fun
    } else {
      crps.fun.y.matrix <- input$crps.fun.y.matrix
    }
    if (!("inf.fun" %in% names(input))) {
      inf.crps.fun <- \(x,i) {input$fun(x)}
    } else {
      inf.crps.fun <- input$inf.fun
    }
  ...
  if (is.matrix(mu) || is.matrix(sd) || is.matrix(w)) {
    ...
    sample.fun <- \(n, n.obs) { rcdf_rf(points.cdf = points.cdf[n.obs][[1]], n = n) }
